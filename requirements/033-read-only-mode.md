...
## Description
The harness must support a `--read-only` flag that, when specified, restricts the coding agent to only read-only operations. In read-only mode, the agent has access only to the `read_file` and `list_files` tools, which are safe for read-only operations. All write-modifying tools (such as `bash`, `write_file`, `insert_lines`, `replace_text`, and `patch`) are disabled and not available to the agent.

## Acceptance Criteria
- [ ] `--read-only` flag is available as a command-line option
- [ ] When `--read-only` is specified, only `read_file` and `list_files` tools are available
...
- [ ] Tool definitions sent to the LLM only include `read_file` and `list_files`
...
# Read-only mode (only read_file and list_files)
...
2. Only list and describe the `read_file` and `list_files` tools
3. Explicitly state that no write operations are allowed
4. Inform the user that the agent can only read files and list directory contents
